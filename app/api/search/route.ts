...
import { publicText, type ResourceRecommendation } from "@/lib/corpus";
...
function recommendations(hits: Awaited<ReturnType<typeof searchQdrant>>, locale: Locale) {
  ...
  return hits.flatMap((hit): ResourceRecommendation[] => {
    ...
    return [{
      resourceCode,
      formation: publicText(payload.formation),
      blockCode: String(payload.block_code ?? ""),
      blockTitle: publicText(payload.block_title),
      ...
      hasPrivateDocument: Boolean(String(payload.private_document_url ?? "").trim()),
    }];
  }).slice(0, 8);
}
