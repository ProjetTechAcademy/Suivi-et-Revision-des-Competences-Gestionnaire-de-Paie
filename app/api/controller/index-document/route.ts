...
export async function POST(request: NextRequest) {
  ...
  try { payload = await request.json() as LegacyPayload; }
  ...
  try {
    ...
    return NextResponse.json({ error: message === "INVALID_BASE64" ? "Contenu base64 invalide" : "Échec de l’indexation du document" }, { status: message === "INVALID_BASE64" ? 400 : message.startsWith("OPENAI_") ? 502 : 500 });
  }
}
