...
async function askCampus(payload: { query: string; pulse?: string; resourceCode?: string }) {
  const response = await fetch("/api/search", {
    method: "POST",
    headers: { "Content-Type": "application/json", "x-campus-access-token": sessionStorage.getItem("campus-access-token") || "" },
...
function Search({ answer, setAnswer, pulse, clearPulse }: { answer: Answer | null; setAnswer: (answer: Answer) => void; pulse: string; clearPulse: () => void }) {
  ...
  useEffect(() => setAccessToken(sessionStorage.getItem("campus-access-token") || ""), []);
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    if (!query.trim()) return;
    setError(""); setLoading(true);
    try {
      sessionStorage.setItem("campus-access-token", accessToken);
      const data = await askCampus({ query, pulse });
      setAnswer(data);
  ...
  return (
    <section className="searchArea shell" aria-label="Recherche principale">
      <label className="accessField"><span>Accès au Campus privé</span><input type="password" value={accessToken} onChange={(event) => setAccessToken(event.target.value)} autoComplete="current-password" placeholder="Votre code d’accès" /></label>
...
function Explorer({ setAnswer }: { setAnswer: (answer: Answer) => void }) {
  ...
  const requestResource = async (mode: "summary" | "mastery") => {
    const query = mode === "summary" ? `Résume la ressource « ${resource} ».` : `Quels sont les points à maîtriser dans la ressource « ${resource} » ?`;
    try {
      setAnswer(await askCampus({ query, resourceCode: `CPA-${String(resource.length).padStart(3, "0")}` }));
      document.querySelector("#fiche")?.scrollIntoView({ behavior: "smooth" });
    } catch (reason) {
      window.alert(reason instanceof Error ? reason.message : "La recherche est momentanément indisponible.");
    }
  };
...
function KnowledgeSheet({ answer }: { answer: Answer | null }) {
  ...
  const text = answer?.summary || "Posez une question pour générer une fiche à partir du corpus Campus PAÏA.";
    ...
    <section className="sheetSection" id="fiche">
      <div className="shell"><h2>Aperçu d’une fiche Campus PAÏA</h2><div className="sheetLayout">
        ...
        <article className="knowledgeCard"><div className="domainRail">PAIE & SOCIAL</div><div className="knowledgeBody">
          ...
          <section id="part-2"><h3>Ce que dit la formation</h3><p>Cette partie restitue le contexte pédagogique trouvé dans les ressources privées, sans exposer les documents d’origine.</p></section>
          ...
          <section id="part-5"><h3>Le réflexe pro</h3><p>Vérifiez les données sources, documentez le contrôle effectué et conservez une trace de la date de vérification.</p></section>
          <section id="part-6"><h3>Glossaire</h3><p>Les termes métier importants de la réponse sont explicités ici lorsqu’ils sont présents dans le corpus.</p></section>
          <section id="part-7"><h3>Sources</h3><p>Campus PAÏA recommande les ressources pertinentes sans révéler leurs URL Drive, identifiants ou chemins privés.</p></section>
          <section id="part-8"><h3>À retenir</h3><p>Retenez les principes essentiels, puis contrôlez les règles susceptibles d’évoluer auprès des sources officielles.</p></section>
        </div></article>
...
export default function CampusApp() {
  ...
  return <><Header /><main><Hero /><Search answer={answer} setAnswer={setAnswer} pulse={pulse} clearPulse={() => setPulse("")} /><Pulses onSelect={selectPulse} /><Shortcuts /><WhyPaia /><Explorer setAnswer={setAnswer} /><KnowledgeSheet answer={answer} /><div id="favoris" /></main><footer><div className="shell"><Brand compact /><p>🌱 Apprendre. Comprendre. Progresser. Ensemble.</p><span>Campus PAÏA · Septembre 2026</span><Image src="/brand/03_MMPA_Circulaire_Logo.png" width={56} height={56} alt="Logo officiel MMPA" /></div></footer><PiaAssistant /></>;
}
