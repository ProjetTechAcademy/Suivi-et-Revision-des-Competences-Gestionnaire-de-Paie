...
export async function replaceVectorStoreFiles(
  apiKey: string,
  vectorStoreId: string,
  predicate: (file: VectorStoreFile) => boolean,
) {
  const files = await listVectorStoreFiles(apiKey, vectorStoreId);
  const matches = files.filter(predicate);
  await Promise.all(matches.map((file) => detachVectorStoreFile(apiKey, vectorStoreId, file.id)));
